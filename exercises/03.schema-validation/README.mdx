...
// https://github.com/edmundhung/conform/issues/211
const FORM_ERROR = ''
...
export async function action({ request }: DataFunctionArgs) {
	...
	const submission = parse(formData, {
		schema: LoginSchema,
		// This makes it so if a field has more than one error, we'll display all of them
		// https://github.com/edmundhung/conform/issues/215
	...
	if (!isAuthenticated) {
		submission.error[FORM_ERROR] = ['Invalid email or password']
	...
	return (
		<Form method="post" {...form.props}>
			<div>
				...
				<input {...conform.input(fields.email, { ariaAttributes: true })} />
